...
//        $dbPass = "hello007";
    ...
    public function deleteCourse($sql)
    {
        $this->connect->query($sql);

    }
    public function editCourse($sql)
    {
        $this->connect->query($sql);
...
//        return $reviewId;
    ...
    //GET STUDENT DEPARTEMENTS
    //////////////////////////////
    public function getHighestGrade($sql)
    {
        $results = mysqli_query($this->connect, $sql);
        if ($results->num_rows) {
            while ($row = $results->fetch_object()) {
            ...
            foreach($records as $result)
            {
                $highestGrade = $result->finalLetterGrade;
            }
        }
    ...
    public function getCoursesTaken($departmentName)
    {
        ...
        $coursesNames='';
        $sql="SELECT courseName, courseNameCode, courseId FROM course WHERE deptId = $departmentID ORDER BY  courseName, courseNameCode ASC ";
        $results= mysqli_query($this->connect, $sql);
        if($results->num_rows){
            while ($row = $results->fetch_object()) {
    ...
    public function getEditCourses($sql){
        ...
        if($results->num_rows){
            ...
            foreach($records as $result)
            {
               ...
               $editCourses = $courseName.'-'.$courseNameCode.'-'.$departmentName;
            }
    ...
    public function getExistingCourses($sql){
        ...
        if($results->num_rows){
            ...
            foreach($records as $result)
            {
                $existingCourse=$result->courseName;
                $existingCourseCode=$result->courseNameCode;
                $existingCourseName=$existingCourse.' '.$existingCourseCode;
                $existingDepartment=$result->deptName;
                $courseId=$result->courseId;
                $courseId =  base64_encode(strtr($courseId, '-_,', '+/='));;
                $existingCourseNames.='<tr>'.
                    '<td>'.$existingCourseName.'</td><td>'.$existingDepartment.'</td><td><a href="../management/courseInformationForm.php?delete='.$courseId.'"><span class="glyphicon glyphicon-trash"></span></a></td><td><a href="../management/courseInformationEditForm.php?edit='.$courseId.'"><span class="glyphicon glyphicon-pencil"></span></a></td>'.
                    '</tr>';
            }
    ...
    public function getResearchName()
{
    ...
    {
        $courseNames='';
        $sql="SELECT * FROM course ORDER BY courseName,courseNameCode ASC";
        $results= mysqli_query($this->connect, $sql);
        if($results->num_rows){
            while ($row = $results->fetch_object()) {
